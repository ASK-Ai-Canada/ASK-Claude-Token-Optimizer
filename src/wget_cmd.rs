...
pub fn run(url: &str, args: &[String], verbose: u8) -> Result<()> {
    ...
    if output.status.success() {
        ...
        println!("{}", msg);
        timer.track(&format!("wget {}", url), "ask wget", &raw_output, &msg);
    } else {
...
pub fn run_stdout(url: &str, args: &[String], verbose: u8) -> Result<()> {
    ...
    if output.status.success() {
        ...
        let mut ask_output = String::new();
        if total > 20 {
            ask_output.push_str(&format!(
                "⬇️ {} ok | {} lines | {}\n",
                compact_url(url),
                total,
                format_size(output.stdout.len() as u64)
            ));
            ask_output.push_str("--- first 10 lines ---\n");
            for line in lines.iter().take(10) {
                ask_output.push_str(&format!("{}\n", truncate_line(line, 100)));
            }
            ask_output.push_str(&format!("... +{} more lines", total - 10));
        } else {
            ask_output.push_str(&format!("⬇️ {} ok | {} lines\n", compact_url(url), total));
            for line in &lines {
                ask_output.push_str(&format!("{}\n", line));
            }
        }
        ...
        let msg = format!("⬇️ {} FAILED: {}", compact_url(url), error);
        println!("{}", msg);
        timer.track(&format!("wget -O - {}", url), "ask wget -o", &stderr, &msg);
    }
