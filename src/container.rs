...
fn docker_ps(_verbose: u8) -> Result<()> {
    ...
    if stdout.trim().is_empty() {
        ask.push_str("🐳 0 containers");
        println!("{}", ask);
        timer.track("docker ps", "ask docker ps", &raw, &ask);
        return Ok(());
    }

    let count = stdout.lines().count();
    ask.push_str(&format!("🐳 {} containers:\n", count));
    ...
    if count > 15 {
        ask.push_str(&format!("  ... +{} more", count - 15));
    }
...
fn docker_images(_verbose: u8) -> Result<()> {
    ...
    if lines.is_empty() {
        ask.push_str("🐳 0 images");
        println!("{}", ask);
        timer.track("docker images", "ask docker images", &raw, &ask);
        return Ok(());
    }
    ...
    if lines.len() > 15 {
        ask.push_str(&format!("  ... +{} more", lines.len() - 15));
    }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if container.is_empty() {
        println!("Usage: ask docker logs <container>");
        return Ok(());
    }
...
fn kubectl_pods(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => {
            ask.push_str("☸️  No pods found");
            println!("{}", ask);
            timer.track("kubectl get pods", "ask kubectl pods", &raw, &ask);
            return Ok(());
        }
    };
    ...
    ask.push_str(&format!("☸️  {} pods: {}\n", pods.len(), parts.join(", ")));
    if !issues.is_empty() {
        ask.push_str("⚠️  Issues:\n");
        for issue in issues.iter().take(10) {
            ask.push_str(&format!("  {}\n", issue));
        }
        if issues.len() > 10 {
            ask.push_str(&format!("  ... +{} more", issues.len() - 10));
        }
    }
...
fn kubectl_services(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => {
            ask.push_str("☸️  No services found");
            println!("{}", ask);
            timer.track("kubectl get svc", "ask kubectl svc", &raw, &ask);
            return Ok(());
        }
    };
    ...
    ask.push_str(&format!("☸️  {} services:\n", services.len()));
    ...
    if services.len() > 15 {
        ask.push_str(&format!("  ... +{} more", services.len() - 15));
    }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if pod.is_empty() {
        println!("Usage: ask kubectl logs <pod>");
        return Ok(());
    }
...
pub fn run_docker_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ask docker {} (passthrough)", args_str),
    );
...
pub fn run_compose_ps(verbose: u8) -> Result<()> {
    ...
    let ask = format_compose_ps(&structured);
    println!("{}", ask);
    timer.track("docker compose ps", "ask docker compose ps", &raw, &ask);
    Ok(())
}
...
pub fn run_compose_build(service: Option<&str>, verbose: u8) -> Result<()> {
    ...
    let ask = format_compose_build(&raw);
    println!("{}", ask);
    let svc_label = service.unwrap_or("all");
    timer.track(
        &format!("docker compose build {}", svc_label),
        "ask docker compose build",
        &raw,
        &ask,
    );
    Ok(())
...
pub fn run_compose_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ask docker compose {} (passthrough)", args_str),
    );
...
pub fn run_kubectl_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ask kubectl {} (passthrough)", args_str),
    );
