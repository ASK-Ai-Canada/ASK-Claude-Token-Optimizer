...
/// Check if a command has ASK_DISABLED= prefix in its env prefix portion.
pub fn has_ask_disabled_prefix(cmd: &str) -> bool {
    let trimmed = cmd.trim();
    let stripped = ENV_PREFIX.replace(trimmed, "");
    let prefix_len = trimmed.len() - stripped.len();
    let prefix_part = &trimmed[..prefix_len];
    prefix_part.contains("ASK_DISABLED=")
}

/// Strip ASK_DISABLED=X and other env prefixes, return the actual command.
...
/// Rewrite a raw command to its ASK equivalent.
///
/// Returns `Some(rewritten)` if the command has an ASK equivalent or is already ASK-rewritten.
...
pub fn rewrite_command(cmd: &str, excluded: &[String]) -> Option<String> {
    ...
    // Simple (non-compound) already-ASK command — return as-is.
    // For compound commands that start with "ask" (e.g. "ask git add . && cargo test"),
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("ask ") || trimmed == "ask") {
...
/// Rewrite `head -N file` → `ask read file --max-lines N`.
...
fn rewrite_head_numeric(cmd: &str) -> Option<String> {
    ...
    if let Some(caps) = HEAD_N.captures(cmd) {
        ...
        return Some(format!("ask read {} --max-lines {}", file, n));
    }
...
/// Rewrite `tail` numeric line forms to `ask read ... --tail-lines N`.
...
fn rewrite_tail_lines(cmd: &str) -> Option<String> {
    ...
    for re in [
        ...
        if let Some(caps) = re.captures(cmd) {
            let n = caps.get(1)?.as_str();
            let file = caps.get(2)?.as_str();
            return Some(format!("ask read {} --tail-lines {}", file, n));
        }
    }
...
/// Returns `Some(rewritten)` if matched (including already-ASK pass-through).
...
fn rewrite_segment(seg: &str, excluded: &[String]) -> Option<String> {
    ...
    // Already ASK — pass through unchanged
    if trimmed.starts_with("ask ") || trimmed == "ask" {
        return Some(trimmed.to_string());
    }

    // Special case: `head -N file` / `head --lines=N file` → `ask read file --max-lines N`
    // Must intercept before generic prefix replacement, which would produce `ask read -20 file`.
    // Only intercept when head has a flag (-N, --lines=N, -c, etc.); plain `head file` falls
    // through to the generic rewrite below and produces `ask read file` as expected.
    ...
    let ask_equivalent = match classify_command(trimmed) {
        Classification::Supported { ask_equivalent, .. } => {
    ...
    // Find the matching rule (ask_cmd values are unique across all rules)
    let rule = RULES.iter().find(|r| r.rtk_cmd == ask_equivalent)?;
    ...
    // #345: ASK_DISABLED=1 in env prefix → skip rewrite entirely
    if has_ask_disabled_prefix(trimmed) {
        return None;
    }

    // #196: gh with --json/--jq/--template produces structured output that
    // ask gh would corrupt — skip rewrite so the caller gets raw JSON.
    if rule.rtk_cmd == "ask gh" {
...
mod tests {
    ...
    fn test_classify_rtk_already() {
        assert_eq!(classify_command("ask git status"), Classification::Ignored);
    }
    ...
    fn test_rewrite_compound_and() {
        assert_eq!(
            ...
            Some("ask git add . && ask cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_three_segments() {
        assert_eq!(
            ...
            Some("ask cargo fmt --all && ask cargo clippy --all-targets && ask cargo test".into())
        );
    }
    ...
    fn test_rewrite_already_rtk() {
        assert_eq!(
            rewrite_command("ask git status", &[]),
            Some("ask git status".into())
        );
    }
    ...
    fn test_rewrite_background_single_amp() {
        assert_eq!(
            ...
            Some("ask cargo test & ask git status".into())
        );
    }
    ...
    fn test_rewrite_with_env_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh ask git push".into())
        );
    }
    ...
    fn test_rewrite_cat_file() {
        assert_eq!(
            ...
            Some("ask read src/main.rs".into())
        );
    }
    ...
    fn test_rewrite_pipe_first_only() {
        ...
        assert_eq!(
            ...
            Some("ask git log -10 | grep feat".into())
        );
    }
    ...
    fn test_rewrite_mixed_compound_partial() {
        // First segment already ASK-rewritten, second gets rewritten
        assert_eq!(
            rewrite_command("ask git add . && cargo test", &[]),
            Some("ask git add . && ask cargo test".into())
        );
    }
    ...
    fn test_rewrite_rtk_disabled_curl() {
        assert_eq!(
            rewrite_command("ASK_DISABLED=1 curl https://example.com", &[]),
            None
        );
    }
    ...
    fn test_rewrite_rtk_disabled_git_status() {
        assert_eq!(rewrite_command("ASK_DISABLED=1 git status", &[]), None);
    }

    #[test]
    fn test_rewrite_rtk_disabled_multi_env() {
        assert_eq!(
            rewrite_command("FOO=1 ASK_DISABLED=1 git status", &[]),
            None
        );
    }
    ...
    fn test_rewrite_non_rtk_disabled_env_still_rewrites() {
        assert_eq!(
            ...
            Some("SOME_VAR=1 ask git status".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_pipe() {
        assert_eq!(
            ...
            Some("ask cargo test 2>&1 | head".into())
        );
    }
    ...
    fn test_rewrite_redirect_plain_2_devnull() {
        ...
        assert_eq!(
            ...
            Some("ask git status 2>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_and() {
        assert_eq!(
            ...
            Some("ask cargo test 2>&1 && echo done".into())
        );
    }
    ...
    fn test_rewrite_redirect_amp_gt_devnull() {
        assert_eq!(
            ...
            Some("ask cargo test &>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_head_numeric_flag() {
        // head -20 file → ask read file --max-lines 20 (not ask read -20 file)
        assert_eq!(
            rewrite_command("head -20 src/main.rs", &[]),
            Some("ask read src/main.rs --max-lines 20".into())
        );
    }
    ...
    fn test_rewrite_head_lines_long_flag() {
        assert_eq!(
            ...
            Some("ask read src/lib.rs --max-lines 50".into())
        );
    }

    #[test]
    fn test_rewrite_head_no_flag_still_rewrites() {
        // plain `head file` → `ask read file` (no numeric flag)
        assert_eq!(
    ...
    fn test_rewrite_tail_numeric_flag() {
        assert_eq!(
            ...
            Some("ask read src/main.rs --tail-lines 20".into())
        );
    }
    ...
    fn test_rewrite_tail_n_space_flag() {
        assert_eq!(
            ...
            Some("ask read src/lib.rs --tail-lines 12".into())
        );
    }
    ...
    fn test_rewrite_tail_lines_long_flag() {
        assert_eq!(
            ...
            Some("ask read src/lib.rs --tail-lines 7".into())
        );
    }
    ...
    fn test_classify_cargo_install() {
        assert!(matches!(
            classify_command("cargo install ask"),
            Classification::Supported {
                ask_equivalent: "ask cargo",
                ..
            }
    ...
    fn test_rewrite_diff() {
        assert_eq!(
            ...
            Some("ask diff file1.txt file2.txt".into())
        );
    }
    ...
    fn test_rewrite_cargo_install() {
        assert_eq!(
            rewrite_command("cargo install ask", &[]),
            Some("ask cargo install ask".into())
        );
    }
    ...
    fn test_rewrite_kubectl_describe() {
        assert_eq!(
            ...
            Some("ask kubectl describe pod mypod".into())
        );
    }
    ...
    fn test_rewrite_docker_run() {
        assert_eq!(
            ...
            Some("ask docker run --rm ubuntu bash".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_ps() {
        assert_eq!(
            ...
            Some("ask docker compose ps".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_logs() {
        assert_eq!(
            ...
            Some("ask docker compose logs web".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_build() {
        assert_eq!(
            ...
            Some("ask docker compose build".into())
        );
    }
    ...
    fn test_rewrite_aws_ec2() {
        assert_eq!(
            ...
            Some("ask aws ec2 describe-instances --region us-east-1".into())
        );
    }
    ...
    fn test_rewrite_psql() {
        assert_eq!(
            ...
            Some("ask psql -U postgres -d mydb".into())
        );
    }
    ...
    fn test_classify_golangci_lint() {
        assert!(matches!(
            ...
            Classification::Supported {
                ask_equivalent: "ask golangci-lint",
                ..
            }
        ));
    }
    ...
    fn test_rewrite_golangci_lint() {
        assert_eq!(
            ...
            Some("ask golangci-lint run ./...".into())
        );
    }
    ...
    fn test_rewrite_prisma() {
        assert_eq!(
            ...
            Some("ask prisma migrate dev".into())
        );
    }
    ...
    fn test_rewrite_prettier() {
        assert_eq!(
            ...
            Some("ask prettier --check src/".into())
        );
    }
    ...
    fn test_rewrite_compound_or() {
        ...
        assert_eq!(
            ...
            Some("ask cargo test || ask cargo build".into())
        );
    }
    ...
    fn test_rewrite_compound_semicolon() {
        assert_eq!(
            ...
            Some("ask git status; ask cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_pipe_raw_filter() {
        ...
        assert_eq!(
            ...
            Some("ask cargo test | grep FAILED".into())
        );
    }
    ...
    fn test_rewrite_compound_four_segments() {
        assert_eq!(
            ...
            Some(
                "ask cargo fmt --all && ask cargo clippy && ask cargo test && ask git status"
                    .into()
            )
        );
    }
    ...
    fn test_rewrite_env_var_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh ask git push origin main".into())
        );
    }
    ...
    fn test_rewrite_find_with_flags() {
        assert_eq!(
            ...
            Some("ask find . -name '*.rs' -type f".into())
        );
    }
    ...
    #[test]
    fn test_all_rules_have_valid_ask_cmd() {
        for rule in RULES {
            assert!(!rule.rtk_cmd.is_empty(), "Rule with empty rtk_cmd found");
            assert!(
                rule.rtk_cmd.starts_with("ask "),
                "ask_cmd '{}' must start with 'ask '",
                rule.rtk_cmd
            );
            assert!(
    ...
    fn test_rewrite_compound_partial_exclude() {
        ...
        assert_eq!(
            ...
            Some("ask git status && curl https://api.example.com".into())
        );
    }
    ...
    fn test_rewrite_gh_without_json_still_works() {
        assert_eq!(
            ...
            Some("ask gh pr list".into())
        );
    }

    // --- #508: ASK_DISABLED detection helpers ---

    #[test]
    fn test_has_ask_disabled_prefix() {
        assert!(has_ask_disabled_prefix("ASK_DISABLED=1 git status"));
        assert!(has_ask_disabled_prefix("FOO=1 ASK_DISABLED=1 cargo test"));
        assert!(has_ask_disabled_prefix(
            "ASK_DISABLED=true git log --oneline"
        ));
        assert!(!has_ask_disabled_prefix("git status"));
        assert!(!has_ask_disabled_prefix("ask git status"));
        assert!(!has_ask_disabled_prefix("SOME_VAR=1 git status"));
    }

    #[test]
    fn test_strip_disabled_prefix() {
        assert_eq!(
            strip_disabled_prefix("ASK_DISABLED=1 git status"),
            "git status"
        );
        assert_eq!(
            strip_disabled_prefix("FOO=1 ASK_DISABLED=1 cargo test"),
            "cargo test"
        );
