...
/// ASK support status for a command.
...
pub enum RtkStatus {
    ...
    Passthrough,
    /// ASK doesn't handle this command at all
    NotSupported,
}
...
/// A supported command that ASK already handles.
#[derive(Debug, Serialize)]
pub struct SupportedEntry {
    pub command: String,
    pub count: usize,
    pub ask_equivalent: &'static str,
...
/// An unsupported command not yet handled by the optimizer.
#[derive(Debug, Serialize)]
...
pub struct DiscoverReport {
    ...
    pub ask_disabled_count: usize,
    pub ask_disabled_examples: Vec<String>,
}
...
pub fn format_text(report: &DiscoverReport, limit: usize, verbose: bool) -> String {
    ...
    out.push_str("ASK Discover -- Savings Opportunities\n");
    ...
    if report.supported.is_empty() && report.unsupported.is_empty() {
        out.push_str("\nNo missed savings found. ASK usage looks good!\n");
        return out;
    }

    // Missed savings
    if !report.supported.is_empty() {
        out.push_str("\nMISSED SAVINGS -- Commands ASK already handles\n");
        out.push_str(&"-".repeat(72));
        out.push('\n');
        out.push_str(&format!(
            "{:<24} {:>5}    {:<18} {:<13} {:>12}\n",
            "Command", "Count", "ASK Equivalent", "Status", "Est. Savings"
        ));
    ...
    if !report.unsupported.is_empty() {
        ...
        out.push_str(&"-".repeat(52));
        out.push('\n');
        out.push_str("-> github.com/ask-ai/ask/issues\n");
    }

    // ASK_DISABLED bypass warning
    if report.ask_disabled_count > 0 {
        out.push_str(&format!(
            "\nASK_DISABLED BYPASS -- {} commands ran without filtering\n",
            report.ask_disabled_count
        ));
        out.push_str(&"-".repeat(72));
        out.push('\n');
        out.push_str("These commands used ASK_DISABLED=1 unnecessarily:\n");
        if !report.ask_disabled_examples.is_empty() {
            out.push_str(&format!("  {}\n", report.ask_disabled_examples.join(", ")));
        }
        out.push_str("-> Remove ASK_DISABLED=1 to recover token savings\n");
    }
