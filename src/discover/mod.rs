...
use registry::{
    category_avg_tokens, classify_command, has_ask_disabled_prefix, split_command_chain,
...
pub fn run(
    ...
    let mut ask_disabled_count: usize = 0;
    let mut ask_disabled_cmds: HashMap<String, usize> = HashMap::new();
    ...
    for session_path in &sessions {
        ...
        for ext_cmd in &extracted {
            ...
            for part in parts {
                ...
                // Detect ASK_DISABLED= bypass before classification
                if has_ask_disabled_prefix(part) {
                    let actual_cmd = strip_disabled_prefix(part);
                    // Only count if the underlying command is one ASK supports
                    match classify_command(actual_cmd) {
                        Classification::Supported { .. } => {
                            ask_disabled_count += 1;
                            let display = truncate_command(actual_cmd);
                            *ask_disabled_cmds.entry(display).or_insert(0) += 1;
                        }
                        _ => {
                            // ASK_DISABLED on unsupported/ignored command — not interesting
                        }
                    }
                    continue;
                }
                ...
                match classify_command(part) {
                    Classification::Supported {
                        ...
                        let bucket = supported_map.entry(ask_equivalent).or_insert_with(|| {
                            SupportedBucket {
                                ask_equivalent,
                                category,
                                count: 0,
    ...
    let mut supported: Vec<SupportedEntry> = supported_map
        ...
        .map(|bucket| {
            ...
            SupportedEntry {
                ...
                count: bucket.count,
                ask_equivalent: bucket.ask_equivalent,
    ...
    // Build ASK_DISABLED examples sorted by frequency (top 5)
    let ask_disabled_examples: Vec<String> = {
        let mut sorted: Vec<_> = ask_disabled_cmds.into_iter().collect();
