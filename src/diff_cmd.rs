...
pub fn run(file1: &Path, file2: &Path, verbose: u8) -> Result<()> {
    ...
    if diff.added == 0 && diff.removed == 0 {
        ask.push_str("✅ Files are identical");
        println!("{}", ask);
        timer.track(
    ...
    for change in diff.changes.iter().take(50) {
        match change {
            DiffChange::Added(ln, c) => ask.push_str(&format!("+{:4} {}\n", ln, truncate(c, 80))),
            DiffChange::Removed(ln, c) => ask.push_str(&format!("-{:4} {}\n", ln, truncate(c, 80))),
            DiffChange::Modified(ln, old, new) => ask.push_str(&format!(
                "~{:4} {} → {}\n",
                ln,
                truncate(old, 70),
                truncate(new, 70)
    ...
    if diff.changes.len() > 50 {
        ask.push_str(&format!("... +{} more changes", diff.changes.len() - 50));
    }
...
pub fn run_stdin(_verbose: u8) -> Result<()> {
    ...
    timer.track("diff (stdin)", "ask diff (stdin)", &input, &condensed);
