...
#[command(
    name = "ask",
    version,
    about = "ASK Token Optimizer — Unified CLI proxy + semantic compression",
    long_about = "Merges ASK command filters with ASK-TO semantic compression. Dual-hook: PreToolUse rewrite + PostToolUse compress.",
...
enum Commands {
    ...
    Find {
        /// All find arguments (supports both ASK and native find syntax)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    ...
    Grep {
        ...
        /// Remove all ASK artifacts (hook, ASK.md, CLAUDE.md reference, settings.json entry)
    ...
    /// Discover missed ASK savings from Claude Code history
    Discover {
    ...
    /// Rewrite a raw command to its ASK equivalent (single source of truth for hooks)
    ///
    /// Exits 0 and prints the rewritten command if supported.
    /// Exits 1 with no output if the command has no ASK equivalent.
    ///
    ...
    Rewrite {
        ...
        /// Accepts multiple args: `ask rewrite ls -al` is equivalent to `ask rewrite "ls -al"`
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },

    /// Start HTTP API server for conversation compression, code extraction, and output filtering
    Serve {},
}
...
/// ASK-only subcommands that should never fall back to raw execution.
/// If Clap fails to parse these, show the Clap error directly.
const ASK_META_COMMANDS: &[&str] = &[
    "gain",
    "discover",
    "learn",
    "init",
...
fn run_fallback(parse_error: clap::Error) -> Result<()> {
    ...
    // No args → show Clap's error (user ran just "ask" with bad syntax)
    if args.is_empty() {
        parse_error.exit();
    }

    // ASK meta-commands should never fall back to raw execution.
    // e.g. `ask gain --badtypo` should show Clap's error, not try to run `gain` from $PATH.
    if ASK_META_COMMANDS.contains(&args[0].as_str()) {
        parse_error.exit();
    }
    ...
    // TOML filter lookup — bypass with ASK_NO_TOML=1
    ...
    let toml_match = if std::env::var("ASK_NO_TOML").ok().as_deref() == Some("1") {
        None
    } else {
    ...
    if let Some(filter) = toml_match {
        ...
        match status {
            Ok(s) => {
                timer.track_passthrough(&raw_command, &format!("ask fallback: {}", raw_command));
...
fn run_server() -> anyhow::Result<()> {
    ...
    rt.block_on(async {
        ...
        tracing_subscriber::fmt()
            .with_env_filter(
                tracing_subscriber::EnvFilter::try_from_default_env()
                    .unwrap_or_else(|_| "ask_token_optimizer=info,tower_http=info".into()),
            )
            .init();
        ...
        let app = axum::Router::new()
            ...
            .route("/v1/compress/conversation", axum::routing::post(compress_conversation_handler))
            .route("/v1/compress/output", axum::routing::post(compress_output_handler))
            .route("/v1/extract/code", axum::routing::post(extract_code_handler))
            .route("/v1/strip", axum::routing::post(strip_handler))
        ...
        tracing::info!("ASK-Token-Optimizer v{} serving on {}", env!("CARGO_PKG_VERSION"), listen_addr);
...
async fn health_handler() -> axum::Json<serde_json::Value> {
    axum::Json(serde_json::json!({
        "service": "ask-token-optimizer",
        "version": env!("CARGO_PKG_VERSION"),
        "status": "ok"
    }))
}

async fn compress_conversation_handler(
    axum::extract::State(state): axum::extract::State<std::sync::Arc<AppState>>,
    axum::Json(request): axum::Json<conversation::CompressConversationRequest>,
) -> axum::Json<conversation::CompressConversationResponse> {
    let response = conversation::compress_conversation(
        &state.http_client,
        &state.config,
        &request,
    )
...
async fn compress_output_handler(
    axum::Json(request): axum::Json<output::CompressOutputRequest>,
) -> axum::Json<output::CompressOutputResponse> {
    axum::Json(output::compress_output(&request))
}

async fn extract_code_handler(
    ...
    let topic = request.get("topic").and_then(|v| v.as_str()).unwrap_or("");
    let budget = request.get("budget_tokens").and_then(|v| v.as_u64()).unwrap_or(4000) as usize;

    let functions = extract::extract_functions("<input>", content, language);
    let scored = relevance::score_relevance(&functions, topic);
    let selected = relevance::select_context(&scored, budget);

    let result: Vec<serde_json::Value> = selected
        .iter()
        .map(|(score, func)| serde_json::json!({
            "name": func.name,
            "file": func.file,
            "line": func.line,
            "body": func.body,
    ...
    axum::Json(serde_json::json!({
        "functions": result,
        "total_tokens": functions.iter().map(|f| f.tokens).sum::<usize>(),
    }))
}

async fn strip_handler(
    axum::Json(request): axum::Json<serde_json::Value>,
) -> axum::Json<serde_json::Value> {
    let content = request.get("content").and_then(|v| v.as_str()).unwrap_or("");
    let original_tokens = tokens::estimate_tokens(content);
    let cleaned = strip::strip_boilerplate(content);
    let clean_tokens = tokens::estimate_tokens(&cleaned);

    let reduction = if original_tokens == 0 {
        0.0
    } else {
        (1.0 - (clean_tokens as f64 / original_tokens as f64)) * 100.0
    };
    ...
    axum::Json(serde_json::json!({
        ...
        "reduction_pct": format!("{:.1}", reduction),
    }))
}

// ── ASK-TO: CLI hook mode (PostToolUse) ──────────────────────────────────────

/// Run as a Claude Code PostToolUse hook.
///
/// Claude Code sends JSON on stdin: {"tool":"Bash","input":{...},"output":"..."}
/// We extract the "output" field, compress it, replace it, and write the
/// modified JSON to stdout. If stdout is non-empty, Claude Code uses our
/// version instead of the original.
///
/// If stdin is NOT Claude Code JSON (legacy mode), compress the raw text.
fn run_hook_mode() {
    use std::io::Read;
    ...
    // Try to parse as Claude Code hook JSON
    if let Ok(mut hook_json) = serde_json::from_str::<serde_json::Value>(&input) {
        // Extract output and command
        let output_text = match hook_json.get("output").and_then(|v| v.as_str()) {
            Some(s) => s.to_string(),
            None => return,
        };
        ...
        // ── Strategy 1: Try specialized command filter ──────────────────
        // Extract the base command (first word, handle ssh prefix)
        let base_cmd = extract_base_command(&command);
        let mut compressed = None;
        let mut strategy = "rule";

        if !base_cmd.is_empty() {
            // Try running the output through the specialized filter
            // by simulating `ask {base_cmd}` with the output as stdin
            let filter_result = try_specialized_filter(&base_cmd, &output_text);
            if let Some(filtered) = filter_result {
                if filtered.len() < output_text.len() * 80 / 100 {
                    // >20% savings from specialized filter — use it
                    compressed = Some(filtered);
                    strategy = "filter";
                }
            }
        }

        // ── Strategy 2: Try LLM semantic compression via serve endpoint ─
        if compressed.is_none() && output_text.len() > 2000 {
            let serve_url = std::env::var("ASK_SERVE_URL")
                .unwrap_or_else(|_| "http://127.0.0.1:8095".into());
            if let Some(llm_result) = try_llm_compress(&serve_url, &output_text) {
                if llm_result.len() < output_text.len() * 70 / 100 {
                    compressed = Some(llm_result);
                    strategy = "llm";
                }
            }
        }
        ...
        if compressed.is_none() {
            let trimmed = output_text.trim_start();
            let content_type = if (trimmed.starts_with('{') || trimmed.starts_with('['))
                && serde_json::from_str::<serde_json::Value>(trimmed).is_ok()
            {
                "json"
            } else if output_text.contains("ERROR") || output_text.contains("WARN") {
                "log"
            } else {
                "cli"
            };
            let request = output::CompressOutputRequest {
                content: output_text.clone(),
                content_type: content_type.to_string(),
            };
            let result = output::compress_output(&request);
            if result.content.len() < output_text.len() {
                compressed = Some(result.content);
                strategy = "rule";
            }
        }

        // ── Apply result ────────────────────────────────────────────────
        if let Some(ref result) = compressed {
            let original_tokens = tokens::estimate_tokens(&output_text);
            let compressed_tokens = tokens::estimate_tokens(result);
            ...
            stats::record(strategy, original_tokens, compressed_tokens, savings);

            hook_json["output"] = serde_json::Value::String(result.clone());
            print!("{}", serde_json::to_string(&hook_json).unwrap_or_default());

            if savings > 5.0 {
                eprintln!(
                    "[ask-to:{}] {} → {} tokens ({:.1}% saved)",
                    strategy, original_tokens, compressed_tokens, savings
                );
            }
        }
        return;
    ...
    // Legacy mode: raw text compression (not Claude Code JSON)
    if input.len() < 2000 {
        print!("{}", input);
        return;
    }
    ...
    {
        "json"
    } else if input.contains("ERROR") || input.contains("WARN") || input.contains("INFO") || input.contains("DEBUG") {
        "log"
    } else {
        "cli"
    };
    ...
    let savings: f64 = result.metrics.savings_pct.parse().unwrap_or(0.0);
    stats::record(
        content_type,
...
/// Handles: "git status", "ssh root@host git log", "cd /opt && git diff", etc.
fn extract_base_command(cmd: &str) -> String {
    let cmd = cmd.trim();
    // Skip env vars, cd, ssh prefix
    let words: Vec<&str> = cmd.split_whitespace().collect();
    for (i, w) in words.iter().enumerate() {
        // Skip variable assignments
        if w.contains('=') && !w.starts_with('-') { continue; }
        // Skip cd
        if *w == "cd" { continue; }
        // Skip ssh — the real command is after the host
        if *w == "ssh" {
            // Find the host (next non-flag arg), then the command after it
            for j in (i+1)..words.len() {
                if !words[j].starts_with('-') && !words[j].contains('@') {
                    return words[j].to_string();
                }
                if words[j].contains('@') {
                    if j + 1 < words.len() {
                        return words[j+1].trim_matches('"').to_string();
                    }
                }
            }
        }
...
/// Try to compress output using a specialized command filter.
/// Returns Some(compressed) if the filter produced useful output.
fn try_specialized_filter(base_cmd: &str, output: &str) -> Option<String> {
    use std::io::Write;
    use std::process::{Command, Stdio};

    // Run `ask {base_cmd} --hook-filter` with output on stdin
    // But we don't have a --hook-filter flag — instead, use the output module
    // with a content_type hint based on the command
    let content_type = match base_cmd {
        "git" => "cli",
        "docker" | "podman" => "cli",
        "npm" | "pnpm" | "yarn" => "cli",
        "cargo" => "cli",
        "curl" | "wget" => if output.trim_start().starts_with('{') { "json" } else { "cli" },
        "psql" | "mysql" => "cli",
        _ => "cli",
    };
    ...
    let result = output::compress_output(&request);
    if result.content.len() < output.len() {
        Some(result.content)
    } else {
        None
    }
...
/// Try LLM-based semantic compression via the ASK-TO serve endpoint.
/// Returns Some(compressed) or None if serve is unreachable or too slow.
fn try_llm_compress(serve_url: &str, output: &str) -> Option<String> {
    use std::io::{Read, Write};
    use std::net::TcpStream;
    use std::time::Duration;

    // Parse URL manually (no url crate)
    let url = format!("{}/v1/compress/output", serve_url);
    let stripped = url.strip_prefix("http://").unwrap_or(&url);
    let (authority, path) = stripped.split_once('/').unwrap_or((stripped, ""));
    let path = format!("/{}", path);
    let (host, port_str) = authority.split_once(':').unwrap_or((authority, "8095"));
    let port: u16 = port_str.parse().unwrap_or(8095);
    ...
    let json_val: serde_json::Value = serde_json::from_str(resp_body).ok()?;
    json_val.get("content").and_then(|c| c.as_str()).map(String::from)
}

fn main() -> Result<()> {
    // ASK-TO: --hook mode bypasses clap entirely
    let args: Vec<String> = std::env::args().collect();
    // Only honor --hook / --version / -V / version when they are the FIRST argument.
    ...
    if first == "--version" || first == "-V" || first == "version" {
        println!("ask-token-optimizer {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
    ...
    match cli.command {
            ...
            match args[0].as_str() {
                ...
                "prisma" => {
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                timer.track_passthrough(
                                    &format!("npx {}", args_str),
                                    &format!("ask npx {} (passthrough)", args_str),
                                );
                                if !status.success() {
                        ...
                        timer.track_passthrough("npx prisma", "ask npx prisma (passthrough)");
                        if !status.success() {
                            std::process::exit(status.code().unwrap_or(1));
                        }
                    }
        ...
        Commands::Proxy { args } => {
            ...
            // e.g. ask proxy 'head -50 file.php' → cmd=head, args=["-50", "file.php"]
            // e.g. ask proxy 'git log --format="%H %s"' → cmd=git, args=["log", "--format=%H %s"]
            ...
            timer.track(
                ...
                &format!("ask proxy {} {}", cmd_name, cmd_args.join(" ")),
                &full_output,
                &full_output,
            );
...
mod tests {
    ...
    fn test_git_commit_single_message() {
        let cli = Cli::try_parse_from(["ask", "git", "commit", "-m", "fix: typo"]).unwrap();
        match cli.command {
            Commands::Git {
    ...
    fn test_git_commit_am_flag() {
        let cli = Cli::try_parse_from(["ask", "git", "commit", "-am", "quick fix"]).unwrap();
        match cli.command {
            Commands::Git {
                command: GitCommands::Commit { args },
                ..
    ...
    fn test_git_commit_amend() {
        let cli =
            Cli::try_parse_from(["ask", "git", "commit", "--amend", "-m", "new msg"]).unwrap();
        match cli.command {
            Commands::Git {
                command: GitCommands::Commit { args },
                ..
    ...
    fn test_git_global_options_parsing() {
        let cli =
            Cli::try_parse_from(["ask", "git", "--no-pager", "--no-optional-locks", "status"])
                .unwrap();
        match cli.command {
            Commands::Git {
                no_pager,
    ...
    fn test_try_parse_version_is_display_version() {
        match Cli::try_parse_from(["ask", "--version"]) {
    ...
    fn test_try_parse_unknown_subcommand_is_error() {
        match Cli::try_parse_from(["ask", "nonexistent-command"]) {
    ...
    fn test_try_parse_git_with_dash_c_succeeds() {
        let result = Cli::try_parse_from(["ask", "git", "-C", "/path", "status"]);
        assert!(
            result.is_ok(),
    ...
    fn test_gain_failures_flag_parses() {
        let result = Cli::try_parse_from(["ask", "gain", "--failures"]);
        assert!(result.is_ok());
        if let Ok(cli) = result {
            match cli.command {
    ...
    fn test_gain_failures_short_flag_parses() {
        let result = Cli::try_parse_from(["ask", "gain", "-F"]);
        assert!(result.is_ok());
        if let Ok(cli) = result {
            match cli.command {
                Commands::Gain { failures, .. } => assert!(failures),
    ...
    fn test_meta_commands_reject_bad_flags() {
        // ASK meta-commands should produce parse errors (not fall through to raw execution).
        // Skip "proxy" because it uses trailing_var_arg (accepts any args by design).
        for cmd in ASK_META_COMMANDS {
            if *cmd == "proxy" {
                continue;
            }
            let result = Cli::try_parse_from(["ask", cmd, "--nonexistent-flag-xyz"]);
            assert!(
                result.is_err(),
    ...
    fn test_meta_command_list_is_complete() {
        ...
        let meta_cmds_that_parse = [
            ...
            vec!["ask", "init"],
            vec!["ask", "config"],
            vec!["ask", "proxy", "echo", "hi"],
            vec!["ask", "hook-audit"],
            vec!["ask", "cc-economics"],
        ];
    ...
    fn test_rewrite_clap_multi_args() {
        // This is the bug KuSh reported: `ask rewrite ls -al` failed because
        ...
        let cases = vec![
            vec!["ask", "rewrite", "ls", "-al"],
            vec!["ask", "rewrite", "git", "status"],
            vec!["ask", "rewrite", "npm", "exec"],
            vec!["ask", "rewrite", "cargo", "test"],
            vec!["ask", "rewrite", "du", "-sh", "."],
            vec!["ask", "rewrite", "head", "-50", "file.txt"],
        ];
        for args in &cases {
            let result = Cli::try_parse_from(args.iter());
            assert!(
                result.is_ok(),
                "ask rewrite {:?} should parse (was failing before trailing_var_arg fix)",
                &args[2..]
            );
            if let Ok(cli) = result {
                match cli.command {
    ...
    fn test_rewrite_clap_quoted_single_arg() {
        // Quoted form: `ask rewrite "git status"` — single arg containing spaces
        let result = Cli::try_parse_from(["ask", "rewrite", "git status"]);
        assert!(result.is_ok());
        if let Ok(cli) = result {
            match cli.command {
                Commands::Rewrite { ref args } => {
