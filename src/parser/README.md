...
This ensures ASK Token Optimizer **never returns false data silently** while maintaining maximum token efficiency.
        ...
        ParseResult::Degraded(data, warnings) => {
            if verbose > 0 {
                for warn in warnings {
                    eprintln!("[ASK Token Optimizer:DEGRADED] {}", warn);
                }
            }
            data.format(mode)
        }
        ParseResult::Passthrough(raw) => {
            eprintln!("[ASK Token Optimizer:PASSTHROUGH] Parser failed, showing truncated output");
            raw
        }
    };
...
```
[ASK Token Optimizer:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[ASK Token Optimizer:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
