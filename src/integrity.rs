...
//!
//! ASK installs a PreToolUse hook (`ask-rewrite.sh`) that auto-approves
...
//! - Manual verification via `ask verify`
//!
//! Reference: SA-2025-ASK-001 (Finding F-01)
...
const HASH_FILENAME: &str = ".ask-hook.sha256";
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `ask init`
    Tampered { expected: String, actual: String },
    /// Hook exists but no stored hash (installed before integrity checks)
    NoBaseline,
    /// Neither hook nor hash file exist (ASK not installed)
    NotInstalled,
...
/// Resolve the default hook path (~/.claude/hooks/ask-rewrite.sh)
pub fn resolve_hook_path() -> Result<PathBuf> {
    dirs::home_dir()
        .map(|h| h.join(".claude").join("hooks").join("ask-rewrite.sh"))
        .context("Cannot determine home directory. Is $HOME set?")
}

/// Run integrity check and print results (for `ask verify` subcommand)
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `ask init`.");
            eprintln!("  This could indicate tampering or a manual edit.");
            eprintln!();
            eprintln!("  To restore: ask init -g --auto-patch");
        ...
        IntegrityStatus::NoBaseline => {
            ...
            println!("      Run `ask init -g` to establish baseline.");
        }
        IntegrityStatus::NotInstalled => {
            println!("SKIP  ASK hook not installed");
            println!("      Run `ask init -g` to install.");
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("WARN  hash file exists but hook is missing");
            eprintln!("      Run `ask init -g` to reinstall.");
        }
    }
...
/// re-run `ask init -g --auto-patch` to re-establish the baseline.
pub fn runtime_check() -> Result<()> {
    match verify_hook()? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            eprintln!("ask: hook integrity check FAILED");
            eprintln!(
                "  Expected hash: {}...",
            ...
            eprintln!();
            eprintln!("  The hook at ~/.claude/hooks/ask-rewrite.sh has been modified.");
            eprintln!("  This may indicate tampering. ASK will not execute.");
            eprintln!();
            eprintln!("  To restore:  ask init -g --auto-patch");
            eprintln!("  To inspect:  ask verify");
            std::process::exit(1);
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("ask: warning: hash file exists but hook is missing");
            eprintln!("  Run `ask init -g` to reinstall.");
...
mod tests {
    ...
    fn test_store_and_verify_ok() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("ask-rewrite.sh");
    ...
    fn test_verify_orphaned_hash() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("ask-rewrite.sh");
        let hash_file = temp.path().join(".ask-hook.sha256");

        // Create hash but no hook
        fs::write(
            &hash_file,
            "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2  ask-rewrite.sh\n",
        )
        .unwrap();
    ...
    fn test_store_hash_creates_sha256sum_format() {
        ...
        // Format: "<64 hex chars>  ask-rewrite.sh\n"
        assert!(content.ends_with("  ask-rewrite.sh\n"));
        let parts: Vec<&str> = content.trim().splitn(2, "  ").collect();
        assert_eq!(parts.len(), 2);
        assert_eq!(parts[0].len(), 64);
        assert_eq!(parts[1], "ask-rewrite.sh");
    }
    ...
    fn test_invalid_hash_file_rejected() {
        ...
        fs::write(&hook, "test").unwrap();
        fs::write(&hash_file, "not-a-valid-hash  ask-rewrite.sh\n").unwrap();
