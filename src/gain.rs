...
// hook_check removed — ASK-TO manages its own hooks
...
pub fn run(
    ...
    if summary.total_commands == 0 {
        ...
        println!("Run some ask commands to start tracking savings.");
        return Ok(());
    }
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        let title = if project_scope.is_some() {
            "ASK Token Savings (Project Scope)"
        } else {
            "ASK Token Savings (Global Scope)"
        };
        ...
        // Hook status check removed — ASK-TO manages hooks via setup.sh

        // Lightweight ASK_DISABLED bypass check (best-effort, silent on failure)
...
/// Lightweight scan of recent Claude Code sessions for ASK_DISABLED= overuse.
...
fn check_rtk_disabled_bypass() -> Option<String> {
    ...
    use crate::discover::registry::has_ask_disabled_prefix;
    ...
    for session_path in &sessions {
        ...
        for ext_cmd in &extracted {
            total_bash += 1;
            if has_ask_disabled_prefix(&ext_cmd.command) {
                bypassed += 1;
            }
        }
    }
    ...
    if pct > 10.0 {
        Some(format!(
            "⚠️  {} commands ({:.0}%) used ASK_DISABLED=1 unnecessarily — run `ask discover` for details",
            bypassed, pct
        ))
    } else {
        None
...
fn show_failures(tracker: &Tracker) -> Result<()> {
    ...
    println!("{}", styled("ASK Parse Failures", true));
    println!("{}", "═".repeat(60));
    println!();
