...
/// Check if args contain --json flag (user wants specific JSON fields, not ASK filtering)
...
pub fn run(subcommand: &str, args: &[String], verbose: u8, ultra_compact: bool) -> Result<()> {
    // When user explicitly passes --json, they want raw gh JSON output, not ASK filtering
...
fn list_prs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track("gh pr list", "ask gh pr list", &stderr, &stderr);
    ...
    timer.track("gh pr list", "ask gh pr list", &raw, &filtered);
    Ok(())
}
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ask gh pr view {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ask gh pr checks {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_status(_verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr status", "ask gh pr status", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr status", "ask gh pr status", &raw, &filtered);
    Ok(())
}
...
fn list_issues(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh issue list", "ask gh issue list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh issue list", "ask gh issue list", &raw, &filtered);
    Ok(())
}
...
fn view_issue(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ask gh issue view {}", issue_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn list_runs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh run list", "ask gh run list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh run list", "ask gh run list", &raw, &filtered);
    Ok(())
}
...
fn run_repo(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh repo view", "ask gh repo view", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh repo view", "ask gh repo view", &raw, &filtered);
    Ok(())
}
...
fn pr_create(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr create", "ask gh pr create", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr create", "ask gh pr create", &stdout, &filtered);
    Ok(())
}
...
fn pr_merge(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr merge", "ask gh pr merge", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr merge", "ask gh pr merge", &raw, &filtered);
    Ok(())
}
...
fn pr_diff(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr diff", "ask gh pr diff", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr diff", "ask gh pr diff", &raw, &filtered);
    Ok(())
}
...
fn run_passthrough_with_extra(cmd: &str, base_args: &[&str], extra_args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(&full_cmd, &format!("ask {} (passthrough)", full_cmd));
...
fn run_passthrough(cmd: &str, subcommand: &str, args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ask {} {} {} (passthrough)", cmd, subcommand, args_str),
    );
...
mod tests {
    ...
    fn test_extract_identifier_with_repo_flag_after() {
        // gh issue view 185 -R ask-ai/ask
        let args: Vec<String> = vec!["185".into(), "-R".into(), "ask-ai/ask".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "ask-ai/ask"]);
    }

    #[test]
    fn test_extract_identifier_with_repo_flag_before() {
        // gh issue view -R ask-ai/ask 185
        let args: Vec<String> = vec!["-R".into(), "ask-ai/ask".into(), "185".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "ask-ai/ask"]);
    }
    ...
    fn test_extract_identifier_only_flags() {
        ...
        let args: Vec<String> = vec!["-R".into(), "ask-ai/ask".into()];
        assert!(extract_identifier_and_extra_args(&args).is_none());
    }
...
[![CI](https://img.shields.io/github/actions/workflow/status/ask-ai/ask/ci.yml)](https://github.com/ask-ai/ask/actions)
[![Coverage](https://img.shields.io/codecov/c/github/ask-ai/ask)](https://codecov.io/gh/ask-ai/ask)
