...
fn run_generate(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track("prisma generate", "ask prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(cmd_name, &format!("ask {}", cmd_name), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track("prisma db push", "ask prisma db push", &raw, &filtered);
