/// TOML-based filter DSL for ASK Token Optimizer.
///
/// Provides a declarative pipeline of 8 stages that can be configured
/// via TOML files. Lookup priority (first match wins):
///   1. `.ask/filters.toml`              — project-local, committable with the repo
///   2. `~/.config/ask/filters.toml`     — user-global, applies to all projects
...
///
/// `ask init` generates a commented template for both levels (project or global).
///
/// Environment variables:
///   - `ASK_NO_TOML=1`     — bypass TOML engine entirely
///   - `ASK_TOML_DEBUG=1`  — print which filter matched and line counts to stderr
///
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
        ...
        // Priority 1: project-local .ask/filters.toml
        if let Ok(content) = std::fs::read_to_string(".ask/filters.toml") {
            match Self::parse_and_compile(&content, "project") {
                Ok(f) => filters.extend(f),
                Err(e) => eprintln!("[ask] warning: .ask/filters.toml: {}", e),
            }
        }

        // Priority 2: user-global ~/.config/ask/filters.toml
        if let Some(config_dir) = dirs::config_dir() {
            let global_path = config_dir.join("ask").join("filters.toml");
            if let Ok(content) = std::fs::read_to_string(&global_path) {
                match Self::parse_and_compile(&content, "user-global") {
                    Ok(f) => filters.extend(f),
                    Err(e) => eprintln!("[ask] warning: {}: {}", global_path.display(), e),
                }
            }
        }
        ...
        match Self::parse_and_compile(builtin, "builtin") {
            Ok(f) => filters.extend(f),
            Err(e) => eprintln!("[ask] warning: builtin filters: {}", e),
        }
    ...
    fn parse_and_compile(content: &str, source: &str) -> Result<Vec<CompiledFilter>, String> {
        ...
        for (name, def) in file.filters {
            match compile_filter(name.clone(), def) {
                ...
                Err(e) => eprintln!("[ask] warning: filter '{}' in {}: {}", name, source, e),
            }
        }
        Ok(compiled)
    }
...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
    ...
    for cmd in RUST_HANDLED_COMMANDS {
        if match_regex.is_match(cmd) {
            eprintln!(
                "[ask] warning: filter '{}' match_command matches '{}' which is already \
...
// ---------------------------------------------------------------------------
// ask verify — inline test execution
// ---------------------------------------------------------------------------
...
fn collect_test_outcomes(
    ...
    let file: TomlFilterFile = match toml::from_str(content) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[ask] warning: TOML parse error during verify: {}", e);
            return;
        }
    };
    ...
    for (name, def) in file.filters {
        ...
        match compile_filter(name.clone(), def) {
            ...
            Err(e) => eprintln!("[ask] warning: filter '{}' compilation error: {}", name, e),
        }
    }
    ...
    for (filter_name, tests) in file.tests {
        ...
        let compiled = match compiled_filters.get(&filter_name) {
            ...
            None => {
                eprintln!(
                    "[ask] warning: [[tests.{}]] references unknown filter",
                    filter_name
                );
                continue;
            }
...
pub fn find_matching_filter(command: &str) -> Option<&'static CompiledFilter> {
    if std::env::var("ASK_TOML_DEBUG").is_ok() {
        eprintln!(
            "[ask:toml] looking up filter for: {:?} ({} filters loaded)",
            command,
    ...
    if std::env::var("ASK_TOML_DEBUG").is_ok() {
        match result {
            Some(f) => eprintln!("[ask:toml] matched filter: '{}'", f.name),
            None => eprintln!("[ask:toml] no filter matched — passthrough"),
        }
    }
    result
}
