...
/// Parsed arguments from either native find or ASK find syntax.
#[derive(Debug)]
struct FindArgs {
    pattern: String,
    path: String,
...
/// Native find flags that ASK cannot handle correctly.
...
/// Parse arguments from raw args vec, supporting both native find and ASK syntax.
///
/// Native find syntax: `find . -name "*.rs" -type f -maxdepth 3`
/// ASK syntax: `find *.rs [path] [-m max] [-t type]`
fn parse_find_args(args: &[String]) -> Result<FindArgs> {
    ...
    if has_unsupported_find_flags(args) {
        anyhow::bail!(
            "ask find does not support compound predicates or actions (e.g. -not, -exec). Use `find` directly."
        );
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            flag if flag.starts_with('-') => {
                eprintln!("ask find: unknown flag '{}', ignored", flag);
            }
            _ => {}
        }
        i += 1;
...
/// Parse ASK syntax: `find <pattern> [path] [-m max] [-t type]`
