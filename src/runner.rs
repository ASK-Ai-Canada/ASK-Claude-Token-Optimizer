...
pub fn run_err(command: &str, verbose: u8) -> Result<()> {
    ...
    if filtered.is_empty() {
        if output.status.success() {
            ask.push_str("✅ Command completed successfully (no errors)");
        } else {
            ask.push_str(&format!(
    ...
    timer.track(command, "ask run-err", &raw, &ask);
    Ok(())
}
...
pub fn run_test(command: &str, verbose: u8) -> Result<()> {
    ...
    timer.track(command, "ask run-test", &raw, &summary);
    Ok(())
}
